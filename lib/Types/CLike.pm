...
of significant decimal digits.  (However, BigInt/Float defaults to 40 digits, which is above the 39 digits for 128-bit numbers, so you
should be safe.)

Char types will validate that it's a single character, using Perl's Unicode-complaint {length} function.  The bit check types will
also check that the ASCII/Unicode code ({ord}) is the right bit length.
...
Since there are so many different aliases in this module, using {:all} (while available) probably isn't a good idea.  So, there are
...
All of these types are basically aliases of parameterized types in [Types::Numbers].  Caveats of those types apply here.  The types
used are as follows:
...
Some types are also used by [Types::Standard] and [Types::Numbers], namely {Int} and {Char}.  So be careful not to import them at
the same time, as they have different meanings.

== Differences between CLike and real data types

Most C-based languages use a {char} type to indicate both an 8-bit number and a single character, as all strings in those languages
...
numbers.  Thus, to separate the validation of the two, the term {Byte} or {Octet} means the numeric 8-bit types, and the term
{Char} means the single character string types.

The term {long} in C/C++ is ambiguous depending on the bits of the OS: 32-bits for 32-bit OSs and 64-bits for 64-bit OSs.  Since
the 64-bit version makes more sense (ie: {short < int < long}), that is the designation chosen.  To avoid confusion, you can just use
{LongLong} and {ULongLong}.

The confusion is even worse for float types, with the {long} modifier sometimes meaning absolutely nothing in certain hardware
platforms.  {Long} isn't even used in this module for those types, in favor of IEEE 754's "Extended" keyword.
...
float and test for Inf/NaN if you don't want these.  Furthermore, the "Perl safe" scalar tests for floats include checks to make sure
it supports Inf/NaN.  However, the odds of it NOT supporting those (since Perl should be using IEEE 754 floats for NV) are practically
zero.
...
me know if I'm missing *any* type found in the wild.)

= HISTORY

This module started out as [MooX::Types::CLike].  Since [Type::Tiny] came about, that module has been translated to work with TT's
API.  Both [Types::Numbers] and this module are the result.
